...
class DomesticNewsFragment: Fragment(R.layout.fragment_domestic_news), NewsListInterface {

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        lifecycleScope.launchWhenCreated {
            progress_bar_domestic_news.visibility = View.VISIBLE

            val response = try {
                RetrofitObject.newsApiObject.getFootballNewsPosts("news-new")

            } catch (ioError: IOException){
                progress_bar_domestic_news.visibility = View.GONE
            ...
            if(response.isSuccessful && response.body() != null)
                setRecyclerViewData(response.body()!![1].getFootballPostModel())
    ...
    private fun setRecyclerViewData(item: List<PostModel>){
        recycler_view_domestic_news.layoutManager =
            LinearLayoutManager(requireActivity(), RecyclerView.VERTICAL, false)

        recycler_view_domestic_news.adapter = NewsListAdapter(
            item,
            this@DomesticNewsFragment
        )
    ...
    override fun onNewsItemClicked(news_item: PostModel) {
        requireActivity().supportFragmentManager.beginTransaction()
            .replace(R.id.fragment_container_domestic_news, NewsDetailFragment.getNewsDetailFragment(news_item))
            .addToBackStack(null)
            .commit()
    }
}